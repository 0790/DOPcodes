...
#number of neurons in each layer N = 200
...
#size of weight matrix : w1 = (700,200) w2 = (200,20)
...
v1 = torch.empty((N,N), dtype = datatype , device = device , requires_grad= True) #transpose of this is multiplied with incoming spikes
loss_hist = []
...
weight_scale = 0.2
if os.path.isfile((basepath+"/Weights/trainedweights200_recurrentnormalTEST1.pt")):
	print('The file is present.')
	w1,w2,v1 = torch.load(basepath+'/Weights/trainedweights200_recurrentnormalTEST1.pt')
else:
	torch.nn.init.normal_(w1, mean=0.0, std=weight_scale/np.sqrt(Nin))
	torch.nn.init.normal_(w2, mean=0.0, std=weight_scale/np.sqrt(N))
	torch.nn.init.normal_(v1, mean=0.0, std=weight_scale/np.sqrt(N))
...
torch.save([rw1,rw2,rv1] , basepath+'/Weights/trainedweights200_recurrentnormalTEST1.pt')
open_file = open((basepath+"/Plots/list200_recurrentnormalTEST1RUN4.pkl"),"ab")
