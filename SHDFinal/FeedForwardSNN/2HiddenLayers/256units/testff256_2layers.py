...
#number of neurons in each layer N = 256
...
#size of weight matrix : w1 = (700,256) w2 = (256,20)
...
w2 = torch.empty((N,N), dtype = datatype , device = device , requires_grad= True) #transpose of this is multiplied with incoming spikes
w3 = torch.empty((N,Nout), dtype = datatype , device = device , requires_grad= True)
loss_hist = []


#normalize using He initialization
#https://pytorch.org/docs/stable/nn.init.html
print("2 layered 256 unit SNN made weight variables and loss histogram")
print("made weight variables and loss histogram")



if os.path.isfile((basepath+"/Weights/trainedweights256_2layersTEST1.pt")):
	print('The file is present.')
	w1,w2,w3 = torch.load(basepath+'/Weights/trainedweights256_2layersTEST1.pt')
else:
...
Nepochs = 200  # 500, 150 for last run 
...
def forwarddynamic(input):
	...
	Isyn2 = torch.zeros((Nbatch,N), dtype= datatype , device = device)
	Umem2 = torch.zeros((Nbatch,N), dtype= datatype , device = device)
	
	Isynnext2 = torch.zeros((Nbatch,N), dtype= datatype , device = device)
	Umemnext2 = torch.zeros((Nbatch,N), dtype= datatype , device = device)
	Urecord2 = []
	Spikerecord1 = []
	outputlayer2 = torch.zeros((Nbatch,N), dtype= datatype , device = device)
	z2_from_l1 = torch.einsum("ijk,kl->ijl" , (Spikerecord, w2))
	for t in range(Ntimesteps):
		z2 = z2_from_l1[:,t]
		outputlayer2 = spikefunction(Umem2)
		resetspike1 = outputlayer2.detach() #what does this do?

		Isynnext2 = lambd*Isyn2 + z2 # I[t+1] = lambda * I[t] + w1*inputspike  (no V matrix for feedforward)
		Umemnext2 = (muh*Umem2+Isyn2)*(1-resetspike1)

		Spikerecord1.append(outputlayer2)
		Urecord2.append(Umem2)

		Umem2 = Umemnext2
		Isyn2 = Isynnext2	


	Urecord2 = torch.stack(Urecord2,dim=1)
	Spikerecord1 = torch.stack(Spikerecord1, dim=1)



	#hidden unit layer2 to output layer, output layer is leaky integrator which don't spike
	#purpose of this layer is to record the membrane potential of the last layer
	z3 = torch.einsum("ijk,kl->ijl" , (Spikerecord1 , w3) )
	Isyn3 = torch.zeros((Nbatch,Nout), dtype= datatype , device = device)
	Umem3 = torch.zeros((Nbatch,Nout), dtype= datatype , device = device)
	output_potential = [Umem3]
	for t in range(Ntimesteps):
		Isynnext3 = lambd*Isyn3 + z3[:,t]
		Umemnext3 = muh* Umem3 + Isyn3

		Isyn3 = Isynnext3
		Umem3 = Umemnext3
		output_potential.append(Umem3)
	output_potential = torch.stack(output_potential,dim=1)
	

	records = [Urecord1 , Urecord2, Spikerecord1]
...
def training(x , y , alpha= alpha , Nepochs = 10):
	...
	for i in range(Nepochs):
		...
		for x_local, y_local in sparse_data_generator_from_hdf5_spikes(X = x,y = y,batch_size= Nbatch, nb_steps= Ntimesteps,nb_units= Nin,max_time= T):
			...
			_,_,spikes = records #this is used in regularization
...
loss_list,[rw1,rw2,rw3] = training(x_train, y_train , Nepochs = Nepochs)
loss_hist.append(loss_list)
print("Training accuracy: %.3f"%(accuracy(x_train,y_train)))
print("Test accuracy: %.3f"%(accuracy(x_test,y_test)))
torch.save([rw1,rw2,rw3] , basepath+'/Weights/trainedweights256_2layersTEST1.pt')
open_file = open((basepath+"/Plots/list256_2layersTEST1RUN3.pkl"),"ab")
