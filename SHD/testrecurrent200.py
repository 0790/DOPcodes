...
N = 200 #per layer  #this is different
...
if os.path.isfile((basepath+"/trained_values/trainedrecuroglr200.pt")):
	print('The file is present.')
	w1,w2,v1 = torch.load(basepath+'/trained_values/trainedrecuroglr200.pt')
...
def forwarddynamic(input):
	...
	for t in range(Ntimesteps):
		z1_recur = z1[:,t] +torch.einsum("jk,kl->jl", (outputlayer1,v1))
		outputlayer1 = spikefunction(Umem1)
		resetspike = outputlayer1.detach()
...
loss_list,[rw1,rw2,rv1] = training(x_train, y_train , Nepochs = Nepochs)
loss_hist.append(loss_list)

print("Training accuracy: %.3f"%(accuracy(x_train,y_train)))
print("Test accuracy: %.3f"%(accuracy(x_test,y_test)))

torch.save([rw1,rw2,rv1] , basepath+'/trained_values/trainedrecuroglr200.pt')
open_file = open(basepath+"/trained_values/trainedhistrecuroglr200.pkl","ab")
pickle.dump(loss_list,open_file)
print("\nFile list dumped\n")
open_file.close()
print("Saved in file")
open_file = open(basepath+ "/trained_values/trainedhistrecuroglr200.pkl","rb")
