...
print(device)
print("time constants of neurons halved and time steps is 100")
...
if os.path.isfile((basepath+"/trained_values/trainednormallr-0.00005.pt")):
	print('The file is present.')
	w1,w2 = torch.load(basepath+'/trained_values/trainednormallr-0.00005.pt')
	print(w1)
	print(w2)
else:
	print("File not present for network with normal weight initialisation") ;
	torch.nn.init.normal_(w1, mean =0.0, std = 0.2/np.sqrt(Nin)) #given as uniform distribution in papers, but normal distribution in spytorch code
	torch.nn.init.normal_(w2, mean = 0.0, std=0.2/np.sqrt(N))
	print("Initialised with numbers from normal distribution")


Nepochs = 500 #usually 500, 100 for final run of 0.00005
...
#/trainednormallr-0.00005.pt
torch.save([rw1,rw2] , basepath+'/trained_values/trainednormallr-0.00005.pt')
open_file = open(basepath+"/trainedhistnormallr-0.00005.pkl","ab")
pickle.dump(loss_list,open_file)
print("\nFile list dumped\n")
open_file.close()
print("Saved in file")
open_file = open(basepath+ "/trained_values/trainedhistnormallr-0.00005.pkl","rb")
